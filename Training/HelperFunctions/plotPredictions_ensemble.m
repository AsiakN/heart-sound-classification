...
validation_fds = fileDatastore(fullfile(pwd, 'validation_set'), 'ReadFcn', @importAudioFile, 'FileExtensions', '.wav', 'IncludeSubfolders', 1);

data_dir = fullfile(pwd, 'validation_set');
...
while hasdata(validation_fds)
    if rand(1)<0.05
        ...
        %current_class = reference_table_validation(strcmp(reference_table_validation.record_name, PCG.filename), :).record_label;
        %actual{i} = arrayfun(@(C) labelMap(C), current_class, 'UniformOutput', 0);
        % Get the predicted classification from the heart sound model
        predicted{i} = classifyHeartSounds_ensemble(signal, fs);
